...
export default function Register() {
    ...
    const validate = () => {
        ...
        if (!formData.email && !formData.email.includes(/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i.test)) {
            validateErrors.email = 'El correo electrónico es requerido o no es válido'
        }

        if (!formData.password || formData.password.length <= 8) {
            validateErrors.password = 'La contraseña es requerida o debe tener más de 8 caracteres'
        }
    ...
    const handleSubmit = async (e) => {
        ...
        return (
          <form className="max-w-md mx-auto p-4 space-y-4">
            ...
            <div>
              <label htmlFor="name" className="block font-semibold">Nombre de usuario:</label>
              <input
                type="text"
                id="name"
                name="name"
